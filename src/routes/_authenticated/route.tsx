...
import { CalendarDays, CalendarHeart, Compass, LayoutDashboard, LogOut, Moon, Settings, Sun, User, Users, Wrench } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated")({
  ...
  beforeLoad: async ({ location }) => {
    ...
    const companyOnly = ["/dashboard", "/appointments", "/clients", "/services"];
...
function MobileBottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 backdrop-blur-md md:hidden">
      <div className={cn("grid", nav.length === 3 ? "grid-cols-3" : "grid-cols-5")}>
        {nav.map((item) => {
          const active = path.startsWith(item.to);
          return (
            <Link
